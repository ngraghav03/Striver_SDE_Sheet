...
class Solution:
    ...
    def findAllSubsetSums(self, arr):
        n = len(arr)
        self.subsetSum(0, 0, n, arr)   # ! Param 0  is starting index and param 1 is starting sum
...
# ? Space Complexity: O(2^N) + O(N) as we are storing the sums of all subsets as for array of N elements, there are 2^N subsets. 
